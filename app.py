...
# -------------------------------------------------------------------
# N2YO fallback -- wordt alleen gebruikt als CelesTrak niet bereikbaar is
# (bijv. tijdelijke IP-blokkade of storing bij CelesTrak).
# CelesTrak blijft de primaire bron; dit is puur een noodvoorziening.
# -------------------------------------------------------------------
N2YO_API_KEY = "769YNL-L3GL9W-P4MZ8R-5TY7"
N2YO_BASE_URL = "https://api.n2yo.com/rest/v1/satellite"
# standaardlocatie voor de N2YO-observer (Enschede); alt in meters
N2YO_OBSERVER_LAT = 52.218187
N2YO_OBSERVER_LNG = 6.89578
N2YO_OBSERVER_ALT = 0
...
def fetch_tle_group(group: str):
    ...
    # Korte timeout: als CelesTrak traag/onbereikbaar is, moet dit snel
    # falen zodat de N2YO-fallback op tijd kan overnemen (voordat Java
    # of de browser de aanvraag zelf al heeft afgebroken).
...
def fetch_position_n2yo(norad_id: int):
    """
    Noodfallback: haalt de actuele positie van een satelliet rechtstreeks
    bij N2YO op (geen TLE-propagatie nodig, N2YO levert al lat/lon/alt).
    Wordt alleen aangeroepen als CelesTrak faalt. Geeft een dict terug in
    hetzelfde formaat als propagate(), of None als het ook hier misgaat.
    """
    url = (
        f"{N2YO_BASE_URL}/positions/{norad_id}/"
        f"{N2YO_OBSERVER_LAT}/{N2YO_OBSERVER_LNG}/{N2YO_OBSERVER_ALT}/1/"
    ...
    return {
        ...
        "norad_id": norad_id,
        "lat": round(p.get("satlatitude", 0.0), 4),
        "lon": round(p.get("satlongitude", 0.0), 4),
        "alt_km": round(p.get("sataltitude", 0.0), 2),
        "speed_km_s": None,   # N2YO positions-endpoint levert geen snelheid
        "heading_deg": None,  # N2YO positions-endpoint levert geen heading
        "period_min": None,
...
def get_iss():
    ...
    Als CelesTrak helemaal niet bereikbaar is (bijv. timeout door een
    tijdelijke blokkade of storing aan hun kant), wordt als laatste
    redmiddel N2YO geraadpleegd. CelesTrak blijft de voorkeursbron;
    N2YO springt alleen bij zodat de tracker online blijft.
    """
    ...
    try:
        ...
        celestrak_error = str(exc)

    if entry is None and celestrak_error is None:
        # 'stations'-groep is wel opgehaald, maar ISS zat er niet in --
        # probeer een directe CelesTrak-query op NORAD-ID.
        try:
    ...
    if entry is not None:
        ...
        celestrak_error = celestrak_error or "SGP4-propagatie mislukt voor ISS"

    # CelesTrak is niet bereikbaar of gaf geen bruikbare data -- val terug op N2YO.
    try:
        pos = fetch_position_n2yo(ISS_NORAD_ID)
        if pos is not None:
            return jsonify({
                "timestamp": time.time(),
                "satellite": pos,
                "warning": f"CelesTrak niet bereikbaar ({celestrak_error}); N2YO-fallback gebruikt",
            })
    except Exception as n2yo_exc:
        return jsonify({
            "error": f"ISS TLE niet gevonden. CelesTrak: {celestrak_error}. N2YO-fallback: {n2yo_exc}"
        }), 502

    return jsonify({"error": f"ISS TLE niet gevonden: {celestrak_error}"}), 502
