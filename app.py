...
def spacetrack_query(path: str) -> str:
    ...
    Robuust tegen verlopen sessies: Space-Track meldt een verlopen sessie
    niet altijd met een nette 401/403 -- soms komt er gewoon een lege of
    onverwachte body terug met status 200. Daarom wordt bij elke fout
    (foutcode EN verdachte/lege body) eenmaal een verse login geforceerd
    en de query herhaald, voordat we het opgeven.
    """
    global _st_logged_in

    def _looks_like_tle(text: str) -> bool:
        # Een geldige TLE-respons bevat minstens één regel die met "1 "
        # of "2 " begint. Een lege body, HTML-foutpagina, of JSON-foutmelding
        # voldoet daar niet aan -- dat beschouwen we als een mislukte query.
        return any(l.startswith("1 ") or l.startswith("2 ") for l in text.splitlines())
    ...
    while attempts_left > 0:
        ...
        try:
            ...
            if resp.status_code in (401, 403):
                ...
                last_error = f"HTTP {resp.status_code} (Space-Track serverfout)"
            elif not resp.ok:
                resp.raise_for_status()
                return resp.text
            elif not _looks_like_tle(resp.text):
                # Status was ok, maar de body is geen geldige TLE-data --
                # meestal een teken dat de sessie stilletjes is verlopen.
                last_error = "onverwachte/lege respons (sessie waarschijnlijk verlopen)"
            else:
                return resp.text
        except requests.RequestException as exc:
            last_error = str(exc)

        # Nog een poging over: forceer een verse login en probeer opnieuw.
        if attempts_left > 0:
            _st_logged_in = False
            spacetrack_login()

    raise RuntimeError(f"Space-Track query mislukt na herinloggen: {last_error}")
